...
from api_providers import APIProviderType, APIProviderConfig, APIResponse, APIProviderStatus
            ...
            try:
                ...
                try:
                    ...
                    if "config" in str(e).lower() or "invalid" in str(e).lower():
                        ...
                        print(f"   ❌ Config {i:2d}: Unexpected error - {type(e).__name__}")
            ...
            for i, config_change in enumerate(config_changes):
                try:
                    ...
                    with patch.object(mycoder.provider_router, 'query') as mock_query:
                        ...
                        response = await mycoder.process_request(f"Test after config change {i}")
                        
                        if response.get("success"):
                            successful_changes += 1
                            print(f"   ✅ Change {i}: {list(config_change.keys())[0]} updated successfully")
                        else:
                            print(f"   ❌ Change {i}: System not responding after change")
                
                except Exception as e:
                    print(f"   🚨 Change {i}: Exception - {type(e).__name__}")
            
            print(f"📊 Configuration change results: {successful_changes}/{len(config_changes)}")
            ...
            with patch.object(mycoder.provider_router, 'query') as mock_query:
                ...
                for i in range(100):
                    ...
                    if response.get("success"):
                        ...
                        if i % 20 == 0:
                            print(f"   ✅ Switch {i:3d}: {response.get('content', '')[:40]}...")
                    else:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, 'query') as mock_query:
                ...
                for i in range(60):  # Test over 60 requests
                    ...
                    if i % 15 == 0:
                        print(f"   Status at {i:2d}: Success rate {successful/(i+1)*100:.1f}%")
            ...
            with patch.object(mycoder.provider_router, 'query') as mock_query:
                ...
                for i in range(500):  # Create 500 concurrent sessions
                    task = mycoder.process_request(
                        f"Explosion test {i}",
                        session_id=f"explosion_session_{i}"
                    )
                    explosion_tasks.append(task)
                
                # Execute all tasks concurrently
                start_time = time.time()
                responses = await asyncio.gather(*explosion_tasks, return_exceptions=True)
                execution_time = time.time() - start_time
                
                # Analyze results
                successful = sum(1 for r in responses if not isinstance(r, Exception) and r.get("success"))
    ...
    async def test_session_corruption_recovery(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, 'query') as mock_query:
                ...
                for i in range(10):
                    await mycoder.process_request(f"Normal session {i}", session_id=f"normal_{i}")
                ...
                corruption_scenarios = [
                    ("missing_keys", lambda s: {k: v for k, v in s.items() if k != "last_context"}),
                    ("invalid_data", lambda s: {**s, "last_context": "not_a_dict"}),
                    ("circular_ref", lambda s: {**s, "circular": s}),
                    ("invalid_timestamp", lambda s: {**s, "updated_at": "not_a_number"}),
                    ("none_values", lambda s: {**s, "total_interactions": None}),
                ]
                
                # Apply corruptions
                session_ids = list(mycoder.session_store.keys())
                for i, (corruption_name, corruption_func) in enumerate(corruption_scenarios):
                    if i < len(session_ids):
                        ...
                        try:
                            ...
                            print(f"   Applied {corruption_name} to session {session_id}")
                        except Exception as e:
                ...
                for i, session_id in enumerate(session_ids[:5]):
                    try:
                        ...
                        print(f"   🚨 Exception during recovery for {session_id}: {type(e).__name__}")
                
                # Test creation of new sessions after corruption
                new_session_response = await mycoder.process_request(
                    "New session after corruption",
                    session_id="post_corruption_session"
                )
                
                print(f"📊 Session corruption recovery results:")
                print(f"   Recovered sessions: {recovery_successful}/5")
                print(f"   New session creation: {'✅' if new_session_response.get('success') else '❌'}")
                print(f"   Final session count: {len(mycoder.session_store)}")
    ...
    async def test_tool_execution_overload(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mycoder = EnhancedMyCoderV2(
                ...
                config={"ollama_local": {"enabled": True}, "system": {"enable_tool_registry": True}}
            )
            ...
            tool_tasks = []
            for i, test_file in enumerate(test_files[:100]):  # Limit to 100 for stress test
    ...
    async def test_tool_registry_resource_limits(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                print(f"   Huge file read: {'✅' if result.success else '❌'} ({read_time:.2f}s)")
                ...
                print(f"   Huge file write: {'✅' if write_result.success else '❌'} ({write_time:.2f}s)")
...
class TestSystemRestartStress:
    ...
    async def test_rapid_initialization_shutdown_cycle(self):
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with patch.object(mycoder.provider_router, 'query') as mock_query:
                        ...
                        if response.get("success"):
                            ...
                            if cycle % 5 == 0:
                                print(f"   ✅ Cycle {cycle:2d}: Success ({cycle_time:.2f}s)")
                        else:
        ...
        print(f"   Successful cycles: {successful_cycles}/{cycle_count} ({success_rate:.1f}%)")
    ...
    async def test_system_recovery_after_corruption(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, 'query') as mock_query:
                ...
                for i in range(5):
                    await mycoder.process_request(f"Initial session {i}", session_id=f"initial_{i}")
                ...
                corruptions = [
                    ...
                    ("provider_router", lambda: setattr(mycoder, 'provider_router', None)),
                ...
                for corruption_name, corruption_func in corruptions:
                    ...
                    try:
                        # Apply corruption
                        original_value = getattr(mycoder, corruption_name.split('_')[0] if '_' in corruption_name else corruption_name)
                        corruption_func()
                        ...
                        try:
                            ...
                            print(f"   🚨 Recovery exception for {corruption_name}: {type(recovery_error).__name__}")
                        
                        # Restore original value for next test
                        setattr(mycoder, corruption_name.split('_')[0] if '_' in corruption_name else corruption_name, original_value)
                    
                    except Exception as setup_error:
                        recovery_results[corruption_name] = "setup_failed"
                        print(f"   🔧 Setup failed for {corruption_name}: {type(setup_error).__name__}")
                ...
                for corruption, result in recovery_results.items():
                    status_emoji = {"recovered": "✅", "failed": "❌", "exception": "🚨", "setup_failed": "🔧"}[result]
                    print(f"   {status_emoji} {corruption}: {result}")
                
                print(f"   Recovery success rate: {recovered_count}/{len(corruptions)} ({recovered_count/len(corruptions)*100:.1f}%)")
                
                # Should recover from most corruptions
                assert recovered_count >= len(corruptions) * 0.6  # At least 60% recovery rate
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", action="store_true",
                       help="Run configuration limit tests")
    parser.add_argument("--session", action="store_true", 
                       help="Run session management limit tests")
    parser.add_argument("--tools", action="store_true",
                       help="Run tool registry limit tests")
