                ...
                def mock_response(prompt, **kwargs):
                    ...
                    return APIResponse(
                        ...
                        provider=random.choice(
                            [APIProviderType.OLLAMA_LOCAL, APIProviderType.CLAUDE_OAUTH]
                        ),
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(num_sessions):
                    await mycoder.process_request(
                        f"Session overflow test {i}", session_id=f"overflow_session_{i}"
                    )

                    # Print progress every 50 sessions
                    if (i + 1) % 50 == 0:
                        print(
                            f"   Created {i + 1}/{num_sessions} sessions, store size: {len(mycoder.session_store)}"
                        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(100):
                    ...
                    if (i + 1) % 20 == 0:
                        print(
                            f"   Processed {i + 1}/100 requests, Success: {successful}, Failed: {failed}"
                        )
...
"""
            ...
            mycoder = EnhancedMyCoderV2(working_directory=working_dir, config=config)
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                print(
                    f"📊 Processing rate: {total_size/processing_time:,.0f} bytes/second"
                )
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(num_operations):
                    ...
                    if (i + 1) % 100 == 0:
                        ...
                        print(
                            f"   Completed {i + 1}/{num_operations} operations, Sessions: {session_count}"
                        )
...
class TestThermalStress:
                ...
                with patch.object(mycoder, "_get_thermal_status") as mock_thermal:
                    mock_thermal.return_value = {
                        ...
                        "status": (
                            ...
                            else "high" if simulated_temp > 75 else "normal"
                        ),
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(20):
                    ...
                    if response.get("success"):
                        successful += 1
                        print(
                            f"   ✅ Request {i}: Success - {response.get('content', '')[:50]}..."
                        )
                    else:
                        thermal_blocked += 1
                        print(
                            f"   🔥 Request {i}: Blocked - {response.get('error', '')}"
                        )
    ...
    async def test_thermal_recovery_stress(self):
        ...
        config = {
            "ollama_local": {"enabled": True},
            "thermal": {"enabled": True, "max_temp": 75, "critical_temp": 82},
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with patch.object(mycoder, "_get_thermal_status") as mock_thermal:
                    mock_thermal.return_value = {
                        ...
                        "status": (
                            "critical"
                            if temp > 82
                            else "high" if temp > 75 else "normal"
                        ),
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                print(
                    f"   Heating phase success rate: {sum(heating_phase)/len(heating_phase)*100:.1f}%"
                )
                print(
                    f"   Critical phase success rate: {sum(critical_phase)/len(critical_phase)*100:.1f}%"
                )
                print(
                    f"   Cooling phase success rate: {sum(cooling_phase)/len(cooling_phase)*100:.1f}%"
                )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(40):
                    ...
                    response = await mycoder.process_request(f"Network stress test {i}")
            ...
            def mock_intermittent_query(prompt, **kwargs):
                ...
                if random.random() < 0.3:
                    ...
                    return APIResponse(
                        ...
                        provider=random.choice(
                            [APIProviderType.CLAUDE_OAUTH, APIProviderType.OLLAMA_LOCAL]
                        ),
                        duration_ms=int(delay * 1000),
                    )
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i, malformed_input in enumerate(malformed_inputs):
                    try:
                        ...
                        exceptions += 1
                        print(
                            f"   🚨 Input {i:2d}: Exception - {type(e).__name__}: {str(e)[:50]}..."
                        )
                ...
                assert (
                    handled_gracefully
                    >= len(
                        [i for i in malformed_inputs if isinstance(i, str) or i is None]
                    )
                    * 0.8
                )
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for batch_size in batch_sizes:
                    try:
                        ...
                        if response.get("success"):
                            print(
                                f"   ✅ Batch {batch_size:3d}: Success ({processing_time:.2f}s)"
                            )
                        else:
                            print(
                                f"   ❌ Batch {batch_size:3d}: Failed ({processing_time:.2f}s)"
                            )
                        ...
                        print(
                            f"   🚨 Batch {batch_size:3d}: Exception - {type(e).__name__}"
                        )
...
if __name__ == "__main__":
    ...
    if args.quick:
        ...
        pytest.main(
            [__file__ + "::TestConcurrencyStress::test_high_concurrent_requests", "-v"]
        )
