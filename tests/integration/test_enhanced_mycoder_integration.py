...
class TestEnhancedMyCoderIntegration:
    ...
    def create_test_config(self):
        ...
        return {
            ...
            "gemini": {"enabled": False},  # Disable to avoid API costs in tests
            "ollama_local": {
                "enabled": True,
            ...
            "thermal": {"enabled": True, "max_temp": 75, "check_interval": 30},
            "system": {"log_level": "DEBUG", "enable_tool_registry": True},
            "debug_mode": True,
        }
    ...
    async def test_basic_initialization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mycoder = EnhancedMyCoderV2(
                working_directory=working_dir, config=self.create_test_config()
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                response = await mycoder.process_request(
                    "Hello, test the fallback system", session_id="test_fallback"
                )
    ...
    async def test_session_management_across_providers(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            mycoder = EnhancedMyCoderV2(
                working_directory=Path(temp_dir), config=self.create_test_config()
            )
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query1:
                ...
                response1 = await mycoder.process_request(
                    "Start conversation", session_id=session_id
                )
    ...
    async def test_configuration_integration(self):
        ...
        config_data = {
            "claude_anthropic": {"enabled": False, "timeout_seconds": 45},
            "ollama_local": {
                "enabled": True,
                "model": "custom-model",
                "timeout_seconds": 90,
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                mycoder = EnhancedMyCoderV2(
                    working_directory=Path(temp_dir), config=config_dict
                )
                ...
                local_provider = mycoder.provider_router._get_provider(
    ...
    async def test_system_status_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                mycoder.provider_router, "health_check_all"
            ) as mock_health:
    ...
    async def test_resource_limits_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            await mycoder.initialize()
            mycoder.mode_manager.current_mode = (
                mycoder.mode_manager.current_mode.__class__.AUTONOMOUS
            )
...
"""
            ...
            mycoder = EnhancedMyCoderV2(working_directory=project_dir, config=config)
    ...
    async def test_large_project_analysis_scenario(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mycoder = EnhancedMyCoderV2(working_directory=project_root, config=config)
            ...
            with patch.object(mycoder.provider_router, "query") as mock_query:
                ...
                for i in range(50):
                    await mycoder.process_request(
                        f"Session test {i}", session_id=f"perf_session_{i}"
                    )
