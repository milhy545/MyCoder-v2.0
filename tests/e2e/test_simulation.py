...
@pytest.mark.asyncio
async def test_network_connectivity(mock_server):
    """Test 1: Verify network status check against mock server"""

    # Configure MyCoder to use local Ollama
    config = {
    ...
    result = await coder.process_request("Check network status")
    ...
    status = coder._check_network_status()
...
@pytest.mark.asyncio
async def test_update_dependencies_simulation(mock_server):
    ...
    # Execute request
    result = await coder.process_request("Update dependencies")
