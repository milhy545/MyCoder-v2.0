...
def test_config(temp_dir: Path) -> dict:
    ...
    return {
        ...
        "thermal": {"enabled": False, "max_temp": 75},  # Disable by default for tests
        "system": {
...
def pytest_runtest_setup(item):
    ...
    if item.get_closest_marker("network") and item.config.getoption(
        "--skip-network", default=False
    ):
        pytest.skip("Network tests skipped")

    # Skip auth tests if not enabled
    if item.get_closest_marker("auth") and not item.config.getoption(
        "--run-auth", default=False
    ):
