...
class TestClaudeOAuthProvider:
    ...
    async def test_get_auth_manager_import_error(self):
        ...
        with patch('builtins.__import__', side_effect=ImportError("claude-cli-auth not found")):
...
class TestAPIProviderRouter:
    ...
    async def test_query_preferred_provider_success(self):
        ...
        with patch.object(router._get_provider(APIProviderType.GEMINI), 'can_handle_request', return_value=True), \
             patch.object(router._get_provider(APIProviderType.GEMINI), 'query') as mock_query:
    ...
    async def test_health_check_all(self):
        ...
        with patch.object(router._get_provider(APIProviderType.CLAUDE_ANTHROPIC), 'health_check', return_value=APIProviderStatus.HEALTHY), \
             patch.object(router._get_provider(APIProviderType.CLAUDE_OAUTH), 'health_check', return_value=APIProviderStatus.DEGRADED), \
             patch.object(router._get_provider(APIProviderType.GEMINI), 'health_check', return_value=APIProviderStatus.UNAVAILABLE), \
             patch.object(router._get_provider(APIProviderType.OLLAMA_LOCAL), 'health_check', return_value=APIProviderStatus.HEALTHY):
    ...
    def test_get_available_providers(self):
        ...
        router._get_provider(APIProviderType.CLAUDE_ANTHROPIC).status = APIProviderStatus.HEALTHY
        router._get_provider(APIProviderType.OLLAMA_LOCAL).status = APIProviderStatus.HEALTHY
        router._get_provider(APIProviderType.GEMINI).status = APIProviderStatus.UNAVAILABLE
