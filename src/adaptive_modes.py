...
class NetworkDetective:
    ...
    async def test_internet_stability(self) -> bool:
        ...
        logger.debug(f"Internet stability: {successful_tests}/{len(test_urls)} "
                    f"tests passed in {test_duration:.1f}s -> {stability}")
        return stability
    
    async def test_orchestrator_connection(self, 
                                        orchestrator_url: str = "http://192.168.0.58:8020") -> bool:
        ...
        try:
            ...
            async with self.session.get(f"{orchestrator_url}/health") as response:
                ...
                if response.status == 200 and response_time < 5.0:
                    ...
                    logger.debug(f"Orchestrator unhealthy: status={response.status}, "
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get("http://localhost:11434/api/tags", 
                    ...
                    if response.status == 200:
                        ...
                        llm_status["models"] = [model["name"] for model in data.get("models", [])]
                        llm_status["codellama_available"] = any("codellama" in model.lower() 
...
class AdaptiveModeManager:
    ...
    def __init__(self, initial_mode: OperationalMode = OperationalMode.FULL):
        ...
        self.mode_configs = {
            OperationalMode.FULL: ModeCapabilities(
                ...
                available_tools=["filesystem", "git", "terminal", "database", 
                               "memory", "transcriber", "research"],
                max_timeout=30,
    ...
    async def evaluate_and_adapt(self):
        ...
        async with NetworkDetective() as detective:
            ...
            self.health_status.internet_stable = await detective.test_internet_stability()
            self.health_status.orchestrator_available = await detective.test_orchestrator_connection()
            self.health_status.claude_auth_working = await detective.test_claude_authentication()
        ...
        if optimal_mode != self.current_mode:
            ...
            logger.info(f"Mode transition needed: {self.current_mode.value} → {optimal_mode.value} "
                       f"({reason})")
    ...
    def _determine_optimal_mode(self) -> OperationalMode:
        ...
        if (self.health_status.internet_stable and 
            self.health_status.orchestrator_available and 
            self.health_status.claude_auth_working and
            self.health_status.local_resources_ok):
            return OperationalMode.FULL
            
        elif (self.health_status.claude_auth_working and 
              self.health_status.local_resources_ok):
    ...
    async def transition_to_mode(self, new_mode: OperationalMode, reason: str = "manual"):
        ...
        async with self._mode_transition_lock:
            ...
            try:
                ...
                logger.info(f"Mode transition completed: {old_mode.value} → {new_mode.value} "
                ...
                try:
                    ...
                    logger.info(f"Restored to {old_mode.value} mode after failed transition")
                except Exception as restore_error:
                    logger.error(f"Failed to restore mode: {restore_error}")
                    self.current_mode = OperationalMode.RECOVERY
                    logger.critical("Forced to RECOVERY mode due to transition failures")
    ...
    async def _initialize_mode(self, mode: OperationalMode):
        ...
        logger.info(f"Initialized {mode.value} mode with capabilities: "
    ...
    async def query_ai(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        if config.claude_auth and self.claude_auth:
            try:
                ...
                logger.error(f"Claude query failed in {self.current_mode.value} mode: {e}")
                # Try to degrade mode if possible
                if self.current_mode == OperationalMode.FULL:
                    await self.transition_to_mode(OperationalMode.DEGRADED, "claude query failed")
            ...
            return {
                "success": False,
                "content": f"AI services unavailable in {self.current_mode.value} mode. "
                          f"Only basic file operations available.",
                "source": "none"
            }
    
    async def _query_local_llm(self, prompt: str, llm_endpoint: str, **kwargs) -> Dict[str, Any]:
            ...
            "mode_history": [
                {
                    ...
                    "timestamp": timestamp,
                    "reason": reason
                } for mode, timestamp, reason in self.mode_history[-10:]  # Last 10 transitions
            ]
        }