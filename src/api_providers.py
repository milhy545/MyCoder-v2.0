...
class GeminiProvider(BaseAPIProvider):
    ...
    def __init__(self, config: APIProviderConfig):
        ...
        self.base_url = config.config.get("base_url", "https://generativelanguage.googleapis.com/v1beta")
...
class OllamaProvider(BaseAPIProvider):
    ...
    async def query(self, 
        ...
        try:
            ...
            provider_type = APIProviderType.OLLAMA_LOCAL if self.is_local else APIProviderType.OLLAMA_REMOTE
    ...
    async def _process_file_context(self, files: List[Path]) -> str:
        ...
        for file_path in files[:3]:  # Limit for smaller models
            try:
                if file_path.exists() and file_path.is_file():
                    content = file_path.read_text(encoding="utf-8")[:2000]  # Smaller limit
...
class APIProviderRouter:
        ...
        for config in configs:
            if config.provider_type in provider_classes:
                try:
                    ...
                    logger.error(f"Failed to initialize {config.provider_type.value}: {e}")
    ...
    async def query(self, 
        ...
        if preferred_provider and preferred_provider in self.fallback_chain:
            ...
            provider_order.extend([p for p in self.fallback_chain if p != preferred_provider])
        else:
    ...
    def _get_provider(self, provider_type: APIProviderType) -> Optional[BaseAPIProvider]:
    ...
    async def configure_thermal_integration(self, thermal_config: Dict[str, Any]):
        ...
        for provider in self.providers:
            if isinstance(provider, OllamaProvider) and provider.is_local:
                ...
                logger.info(f"Configured thermal integration for {provider.config.provider_type.value}")