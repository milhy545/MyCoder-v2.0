...
class BaseTool(ABC):
    ...
    def can_execute_in_mode(self, mode: str) -> bool:
        ...
        mode_mappings = {
            "FULL": [ToolAvailability.ALWAYS, ToolAvailability.FULL_ONLY, 
                    ToolAvailability.DEGRADED_PLUS, ToolAvailability.AUTONOMOUS_PLUS],
            "DEGRADED": [ToolAvailability.ALWAYS, ToolAvailability.DEGRADED_PLUS, 
                        ToolAvailability.AUTONOMOUS_PLUS],
            "AUTONOMOUS": [ToolAvailability.ALWAYS, ToolAvailability.LOCAL_ONLY, 
                          ToolAvailability.AUTONOMOUS_PLUS],
            "RECOVERY": [ToolAvailability.ALWAYS, ToolAvailability.RECOVERY_ONLY, 
...
class FileOperationTool(BaseTool):
    ...
    async def validate_context(self, context: ToolExecutionContext) -> bool:
        ...
        if self.capabilities.requires_filesystem:
            return context.working_directory is not None and context.working_directory.exists()
        return True
...
class ThermalAwareTool(BaseTool):
    ...
    async def _execute_core_logic(self, 
                                 context: ToolExecutionContext,
                                 **kwargs) -> Any:
...
class ToolRegistry:
    ...
    async def execute_tool(self,
        ...
        self._emit_event("tool_pre_execution", {
            "tool_name": tool_name,
            "context": context
        })
        ...
        try:
            ...
            self._emit_event("tool_post_execution", {
                "tool_name": tool_name,
                "result": result,
                "context": context
            })
            ...
            self._emit_event("tool_execution_error", {
                "tool_name": tool_name,
                "error": error_result,
                "context": context
            })
