...
RE_H3 = re.compile(r"<h3[^>]*>(.*?)</h3>", re.I)
RE_P = re.compile(r"<p[^>]*>(.*?)</p>", re.I | re.S)
RE_BR = re.compile(r"<br\s*/?>", re.I)
RE_LI = re.compile(r"<li[^>]*>(.*?)</li>", re.I)
RE_A = re.compile(r'<a[^>]*href="([^"]*)"[^>]*>(.*?)</a>', re.I)
RE_CODE = re.compile(r"<code[^>]*>(.*?)</code>", re.I)
RE_PRE = re.compile(r"<pre[^>]*>(.*?)</pre>", re.I | re.S)
RE_STRONG = re.compile(r"<strong[^>]*>(.*?)</strong>", re.I)
RE_EM = re.compile(r"<em[^>]*>(.*?)</em>", re.I)
