...
def _parse_bash_command(match: re.Match, raw_input: str) -> Command:
    """Parse /bash <command>"""
...
def _parse_file_read(match: re.Match, raw_input: str) -> Command:
    """Parse /file read <path> nebo /read <path>"""
    file_path = match.group(1).strip()
    return Command(tool="file_read", args={"path": file_path}, raw_input=raw_input)


def _parse_file_write(match: re.Match, raw_input: str) -> Command:
...
def _parse_file_list(match: re.Match, raw_input: str) -> Command:
    """Parse /file list [directory]"""
    directory = match.group(1).strip() or "."
    return Command(tool="file_list", args={"directory": directory}, raw_input=raw_input)


def _parse_file_edit(match: re.Match, raw_input: str) -> Command:
...
def _parse_git_status(match: re.Match, raw_input: str) -> Command:
    """Parse /git status"""
    return Command(tool="git_status", args={}, raw_input=raw_input)


def _parse_git_diff(match: re.Match, raw_input: str) -> Command:
    """Parse /git diff [file]"""
...
def _parse_git_log(match: re.Match, raw_input: str) -> Command:
...
def _parse_git_add(match: re.Match, raw_input: str) -> Command:
    """Parse /git add <files>"""
    files = match.group(1).strip()
    return Command(tool="git_add", args={"files": files}, raw_input=raw_input)


def _parse_git_commit(match: re.Match, raw_input: str) -> Command:
...
def _parse_provider_override(match: re.Match, raw_input: str) -> Command:
    """Parse /provider <name>"""
...
COMPILED_PATTERNS = {
    ...
    re.compile(r"^/sh\s+(.+)$", re.IGNORECASE): _parse_bash_command,
    re.compile(r"^/exec\s+(.+)$", re.IGNORECASE): _parse_bash_command,
    re.compile(r"^/file\s+read\s+(.+)$", re.IGNORECASE): _parse_file_read,
    re.compile(r"^/file\s+write\s+(.+)$", re.IGNORECASE): _parse_file_write,
    re.compile(r"^/file\s+list\s*(.*)$", re.IGNORECASE): _parse_file_list,
    re.compile(r"^/read\s+(.+)$", re.IGNORECASE): _parse_file_read,
    re.compile(r"^/write\s+(.+)$", re.IGNORECASE): _parse_file_write,
    re.compile(r"^/edit\s+(.+)$", re.IGNORECASE): _parse_file_edit,
    re.compile(r"^/git\s+status$", re.IGNORECASE): _parse_git_status,
    re.compile(r"^/git\s+diff\s*(.*)$", re.IGNORECASE): _parse_git_diff,
    re.compile(r"^/git\s+log\s*(.*)$", re.IGNORECASE): _parse_git_log,
...
class CommandParser:
    ...
    def parse(self, user_input: str) -> Optional[Command]:
        ...
        # Zkus každý pattern
        for compiled_pattern, parser_func in COMPILED_PATTERNS.items():
