...
class MyCoder:
    ...
    def __init__(
        ...
        logger.info(
            f"MyCoder initialized with working directory: {self.working_directory}"
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if result.get("session_id"):
                await self._update_session_store(
                    result["session_id"], request_context, result
                )
            ...
            logger.info(
                f"Request completed in {duration:.1f}s using {current_mode.value} mode"
            )
            return result
            ...
            if current_mode != OperationalMode.RECOVERY:
                ...
                return await self.process_request(
                    prompt, files, session_id, continue_session, **kwargs
                )
            else:
    ...
    async def _execute_claude_request(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.get("session_id"):
                ...
                claude_kwargs["continue_session"] = context.get(
                    "continue_session", False
                )

            # Execute query
            response = await self.mode_manager.claude_auth.query(
                context["prompt"], **claude_kwargs
            )
    ...
    async def _execute_local_llm_request(
        ...
        result = await self.mode_manager._query_local_llm(
            enhanced_prompt, capabilities.local_llm
        )
            ...
            result.update(
                {
                    ...
                    or f"local_{int(asyncio.get_event_loop().time())}",
                    "source": "local_llm",
                }
            )
    ...
    async def _handle_file_write_request(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_file_list_request(
        self, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_request_failure(self, error: Exception):
        ...
        if current_mode == OperationalMode.FULL:
            ...
            await self.mode_manager.transition_to_mode(
                OperationalMode.AUTONOMOUS, reason
            )
    ...
    async def _update_session_store(
        self, session_id: str, context: Dict[str, Any], result: Dict[str, Any]
    ):
