...
try:
    ...
    from .adaptive_modes import ClaudeAuthManager
...
@click.option(
    "--continue-session", "-c", is_flag=True, help="Continue existing session"
)
...
def status():
    ...
    try:
        ...
        click.echo(
            f"  SDK Interface: {'✅' if config['sdk_interface_initialized'] else '❌'}"
        )
        click.echo(
            f"  CLI Interface: {'✅' if config['cli_interface_initialized'] else '❌'}"
        )
...
def test():
    ...
    async def _test():
        try:
            ...
            response = await claude.query("Say hello briefly", timeout=30)
